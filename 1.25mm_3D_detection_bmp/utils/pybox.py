...
def train(net, train_loader, optimizer, epoch, writer):
    ...
    # 新增代码, 把tensor的divce变为cpu，以转换成array
    for i in range(len(rpn_stats)):
        # a = rpn_stats[i]
        for j in range(len(rpn_stats[i])):
            # b = rpn_stats[i][j]
            if torch.is_tensor(rpn_stats[i][j]):
                rpn_stats[i][j] = rpn_stats[i][j].cpu()
