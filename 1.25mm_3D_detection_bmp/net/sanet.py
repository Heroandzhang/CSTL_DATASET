...
class CropRoi(nn.Module):
    ...
    def forward(self, f, inputs, proposals):
        ...
        for p in proposals:
            ...
            fe1 = comb2[b.item(), :, z_start.item() // 4:z_end // 4, y_start.item() // 4:y_end // 4, x_start.item() // 4:x_end // 4].unsqueeze(0)
            fe1_up = self.up2(fe1)

            fe2 = self.back2(torch.cat((fe1_up, out1[b.item(), :, z_start.item() // 2:z_end // 2, y_start.item() // 2:y_end // 2, x_start.item() // 2:x_end // 2].unsqueeze(0)), 1))
            # fe2_up = self.up3(fe2)
