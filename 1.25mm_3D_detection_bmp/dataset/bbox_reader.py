...
class BboxReader(Dataset):
    def __init__(self, data_dir, set_name, cfg, mode='train'):
        ...
        for fn in self.filenames:  # 此循环用来将csv文件中的所有坐标统计并保存在labels中
            ...
            if np.all(l==0):
                l=np.array([])
            labels.append(l)  # labels中包含zmin,zmax,ymin,ymax,zmin,zmax
            ...
            for i, l in enumerate(labels):
                if len(l) > 0 :
                    for t in l:
                        ...
                        self.bboxes.append([np.concatenate([[i],t])])
            self.bboxes = np.concatenate(self.bboxes, axis=0).astype(np.float32)  # self.bboxes中的存放index,zmin,zmax,ymin,ymax,xmin,xmax，所有值都是float且shape==(结节个数,7)
        self.crop = Crop(cfg)
        a=self.crop
    ...
    def __getitem__(self, idx):
        ...
        if self.mode in ['train', 'val']:
            if not is_random_img:
                ...
                lines = os.listdir(os.path.join(self.data_dir, filename))
                lines = sorted(lines)
                slice_files = [os.path.join(self.data_dir, filename, s) for s in lines]
            ...
            truth_labels = bboxes[:, -1]  # 如果改crop_size，这里会出现bug，提示bboxes是1维却给了两个索引，但实际上bboxes是2维，且调试可以通，但运行报错
    ...
    def __len__(self):
        if self.mode == 'train':
            ...
            print(int(len(self.bboxes) / (1-self.r_rand))//self.cfg['batch_size']*self.cfg['batch_size'],'111')
        elif self.mode =='val':
