...
def main():
    ...
    results_file = "fasterRCNN_mobilenet_result/results{}.txt".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
    ...
    for epoch in range(init_epochs, num_epochs+init_epochs, 1):
        ...
        if epoch in range(num_epochs+init_epochs)[-5:]:
            save_files = {
                ...
                'epoch': epoch}
            torch.save(save_files, "fasterRCNN_mobilenet_result/mobile-model-{}.pth".format(epoch))
